...
//
var process = function(sentences, numberOfTopics, numberOfTermsPerTopic, languages, alphaValue, betaValue, randomSeed, itterations) {
    ...
    if (sentences && sentences.length > 0) {
      ...
      for(var i=0;i<sentences.length;i++) {
          ...
          for(var wc=0;wc<words.length;wc++) {
              ...
              if(vocabOrig[wStemmed]===undefined){
                  vocabOrig[wStemmed] = w;
              }else if (w.length<vocabOrig[wStemmed].length){
                  vocabOrig[wStemmed] = w;
              }
      ...
      var itr = itterations || 10000;
      var removedDocs = []
      documents = documents.filter((doc, docIndex) => { 
          if(doc.length===0){
            removedDocs.push(docIndex)
            return false
          }
          return true
      }); // filter empty documents

      lda.configure(documents,V,itr, 2000, 100, 10, randomSeed);
    ...
    return {result:result, theta:theta, phi:phi, removedDocs:removedDocs, vocab:vocab};
}
